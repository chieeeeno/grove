...
#### 9-1. Frontend 側
- [ ] SettingsDialog コンポーネント作成（モーダル）
- [ ] ダイアログ表示: テーマ選択（M0 ではシステム固定、表示のみ）
- [ ] ダイアログ表示: エディタ選択（M0 では VS Code 固定、表示のみ）
- [ ] ダイアログ表示: 自動更新間隔の変更（5秒 / 10秒 / 30秒）
- [ ] サイドバーの「設定」ボタンからダイアログを開く
- [ ] 設定変更時に store + tauri-plugin-store に保存
...
- [ ] QA チェック項目の洗い出し（全機能・全画面・エッジケースを網羅）
- [ ] QA チェックリストの実施
- [ ] 発見した不具合の修正
